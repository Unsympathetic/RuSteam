...
namespace RuSteam
{
    public partial class CashForm : Form
    {
        ...
        private void btnCash_Click(object sender, EventArgs e)
        {
            ...
            if(MessageBox.Show("Вы точно хотите зарегистрировать эту продажу?", "Продажа", MessageBoxButtons.YesNo, MessageBoxIcon.Question)==DialogResult.Yes)
            {
                getTransno();
                main.loadDailySale();
                ...
                dgvCash.Rows.Clear();
                MessageBox.Show("Продажа успешно оформлена", "Продажа", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if(colName =="Delete")
            {
                if (MessageBox.Show("Вы точно хотите удалить эту продажу?", "Удаление продаж", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.executeQuery("DELETE FROM tbCash WHERE cashid LIKE'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Продажа успешно удалена", title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else if (colName == "Increase")
            {
                ...
                {
                    MessageBox.Show("У вас имеется " + i + "этого продукта", "Продукт закончился ", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
